...
function BiasCard({ id, aiOn }: { id: string; aiOn: boolean }) {
        ...
        <>
          ...
          <div className="flex gap-2 mt-3">
            <button
              ...
              className="flex-1 text-xs font-medium text-ink border border-card-border rounded-lg py-2 hover:border-accent/40 transition-colors"
            >
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {instruments.map((inst) => (
          <BiasCard key={inst.id} id={inst.id} aiOn={ai?.available === true} />
        ))}
      </div>
    </div>
  );
